...
public class DBGestione {
    ...
    if(Pattern.matches("[a-zA-Z_�_�_�_�_�_'_ _]+", nome)==false){                 
    	c++;
    }
    return c;
	}
    ...
    if(Pattern.matches("[a-zA-Z_�_�_�_�_�_'_ _]+", cognome)==false){                 
    	c = 2;
    }
    return c;
	}
    ...
    if(Pattern.matches("[a-zA-Z_�_�_�_�_'_ _]+", via)==false){                 
        c++;
	...
	if(Pattern.matches("[a-zA-Z_�_�_�_�_�_'_ _]+", citta)==false){                 
         c++;
        // System.out.println("La citt� da lei inserita non rispetta il formato richiesto");
    }
	return c;
	}
...
//	    System.out.println("La fornitura creata ha questo identificativo: "+id); 
	...
	public int CheckCitta(String citta) {
		...
		if(rsc.next() == false){
		//System.out.println("La citt� da lei inserita non � presente nel database.Inserirne una corretta.");
		x++;
		}
		}catch (ClassNotFoundException | SQLException ex) {
		Logger.getLogger("com.mysql.cj.jdbc.Driver").log(Level.SEVERE, null, ex);
	...
	public ArrayList<String> CheckTipo(String tipo){
		...
		/*	System.out.println("Inserire il tipo della fornitura di cui si richiedono i report");
			...
			if(Pattern.matches("[1_2_3]+", tipo)==false){                 
				c++;
				//System.out.println("Ci� che ha inserito non rispetta il formato richiesto");
			}
...
public Double GeneraRep(String citta, String tipo, String unita){
	...
	//System.out.println(" I consumi medi mensili dell'ultimo anno per unit� abitativa sono " + rsq.getDouble("Consumi_medi")+" "+unita+" per la citt� di "+citta+" per quanto riguarda la fornitura di "+tipo);
	}
...
public int CheckDataIn(String in) {
	...
	int i = 0;
		try {
			...
			//System.out.println("La data inserita non � valida");
			i++;
		}
...
public ArrayList<String> CheckDataFin(String fin, String in){
		...
		Date today = Date.from(today2.atStartOfDay(defaultZoneId).toInstant());
		int j =0;
		//System.out.println("Inserisci la data di fine (yyyy-mm-dd) da quando vuoi sapere le analytics di questa citt�");
		try {
